#!/usr/bin/env python3
"""Regression tests for integrated nm1 tanh vec-op RTL generation."""
...
class NpuRtlgenVecTanhRegressionTest(unittest.TestCase):
    def test_generates_tanh_enabled_nm1_top_and_runs_vec_smoke(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            cfg_path = root / "cfg.json"
            out_dir = root / "npu" / "rtlgen" / "out"
            vvp_path = root / "npu_top.vvp"
            cfg = json.loads(TANH_CFG.read_text(encoding="utf-8"))
            ...
            self.assertIn("VEC_OP_TANH      = 4'hb", top_v)
            self.assertIn("vec_act_tanh_int8", top_v)
            self.assertIn("VEC_EN_TANH      = 1", top_v)
