...
def _run_guard(design_dir: Path) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            "npu/eval/check_attention_score32_exact_local_temporal_reducer_gqa8_physical_harness_guard.py",
            "--design-dir",
            str(design_dir),
            "--config",
...
def test_gqa8_physical_harness_guard_accepts_checked_in_configs(tmp_path: Path) -> None:
...
def test_gqa8_physical_harness_guard_rejects_stale_top(tmp_path: Path) -> None:
...
def test_gqa8_physical_harness_guard_rejects_missing_proposal_linkage(tmp_path: Path) -> None:
    ...
    result = _run_guard(design_dir)
    assert result.returncode != 0
    assert "report_links proposal_id must be prop_l1_decoder_attention_score32_local_temporal_reducer_gqa8_v1" in result.stderr


def test_gqa8_physical_harness_guard_rejects_missing_source_token(tmp_path: Path) -> None:
    design_dir = _prepare_design_dir(tmp_path, producers=53, mode="source_only")
    top_path = design_dir / "verilog" / "top.v"
    text = top_path.read_text(encoding="utf-8")
    top_path.write_text(
        text.replace("if (shared_beat_count_q == 12'd2047) begin", "if (shared_beat_count_q == 12'd2046) begin"),
        encoding="utf-8",
    )
