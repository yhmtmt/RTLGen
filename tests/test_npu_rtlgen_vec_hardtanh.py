#!/usr/bin/env python3
"""Regression tests for integrated nm1 hard-tanh vec-op RTL generation."""
...
class NpuRtlgenVecHardTanhRegressionTest(unittest.TestCase):
    def test_generates_hardtanh_enabled_nm1_top_and_runs_vec_smoke(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            cfg_path = root / "cfg.json"
            out_dir = root / "npu" / "rtlgen" / "out"
            vvp_path = root / "npu_top.vvp"
            cfg = json.loads(HARDTANH_CFG.read_text(encoding="utf-8"))
            ...
            self.assertIn("VEC_OP_HARDTANH = 4'hd", top_v)
            self.assertIn("vec_act_hardtanh_int8", top_v)
            self.assertIn("VEC_EN_HARDTANH = 1", top_v)
