...
PROD_TMP="$TMP/producer_coupled"
mkdir -p "$PROD_TMP"
cp "$ROOT/examples/config_attention_kv_reducer_folded_producer_coupled.json" "$PROD_TMP/config.json"
(
  cd "$PROD_TMP"
  "$ROOT/build/rtlgen" "$PROD_TMP/config.json"
)

PROD_WRAP_DIR="$TMP/producer_coupled_wrap"
PYTHONPATH="$ROOT/scripts" python3 - "$ROOT/examples/config_attention_kv_reducer_folded_producer_coupled.json" "$PROD_WRAP_DIR" <<'PY'
import json
import sys
from pathlib import Path
import generate_design
...
cfg = json.loads(Path(sys.argv[1]).read_text())
design = generate_design.identify_design(cfg)
assert design["kind"] == "attention_kv_reducer_folded_producer_coupled"
assert design["value_width"] == 64
out = Path(sys.argv[2])
out.mkdir(parents=True, exist_ok=True)
generate_design.generate_wrapper(cfg, str(out), design)
wrapper = out / "attention_kv_reducer_folded_prod_p4_ppc2_l4_v8_s12_a16_wrapper.v"
text = wrapper.read_text()
port_header = text.split(");", 1)[0]
assert "input [63:0] value_fragments" not in port_header
assert "input [47:0] stat_fragments" not in port_header
assert "reg [VALUE_WIDTH-1:0] producer_value_fragments" in text
assert "assign partial_valid = producer_valid" in text
assert "producer_value_fragments[0 +: 8] <=" in text
PY
iverilog -g2012 -s attention_kv_reducer_folded_prod_p4_ppc2_l4_v8_s12_a16_wrapper -t null \
  "$PROD_TMP/attention_kv_reducer_folded_prod_p4_ppc2_l4_v8_s12_a16.v" \
  "$PROD_WRAP_DIR/attention_kv_reducer_folded_prod_p4_ppc2_l4_v8_s12_a16_wrapper.v"

python3 "$ROOT/scripts/run_sweep.py" \
  --configs "$ROOT/runs/designs/activations/attention_kv_reducer_folded_prod_p8_ppc4_l16_v16_s16_a24_wrapper/config_attention_kv_reducer_folded_prod_p8_ppc4_l16_v16_s16_a24.json" \
  --platform nangate45 \
  --sweep "$ROOT/runs/campaigns/activations/attention_kv_reducer_folded/sweeps/nangate45_macro_producer_coupled.json" \
  --dry_run
