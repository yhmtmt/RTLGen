...
module attention_kv_reducer_folded_tb;
  ...
  initial begin
    #1000;
    $display("FAIL attention_kv_reducer_folded: simulation timeout");
    $fatal;
  end
  ...
  initial begin
    ...
    rst_n = 1'b1;
    partial_valid = 1'b0;
    value_fragments = 64'h0;
    stat_fragments = 48'h0;
    reduced_ready = 1'b0;
    ...
    if (!reduced_valid) begin
      $display("FAIL attention_kv_reducer_folded: reduced_valid was not asserted");
      $fatal;
    end
    if ($signed(reduced_value_fragment[15:0]) !== 16'sd12 ||
      ...
      $display(
        "FAIL attention_kv_reducer_folded values: lanes=[%0d,%0d,%0d,%0d] stats=[%0d,%0d]",
    ...
    if (accepted_chunk_count !== 16'd2 || completed_group_count !== 16'd1 ||
        producer_stall_cycles !== 16'd0 || cycle_count === 16'd0 ||
        final_completion_cycle === 16'd0) begin
      $display(
        "FAIL attention_kv_reducer_folded observables: accepted=%0d groups=%0d stalls=%0d cycles=%0d final=%0d",
    ...
    $display("All attention/KV folded reducer tests passed.");
    $finish;
  end
endmodule
