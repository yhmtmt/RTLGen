#!/usr/bin/env python3
"""Regression tests for integrated nm1 hard-sigmoid vec-op RTL generation."""
...
class NpuRtlgenVecHardSigmoidRegressionTest(unittest.TestCase):
    def test_generates_hardsigmoid_enabled_nm1_top_and_runs_vec_smoke(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            cfg_path = root / "cfg.json"
            out_dir = root / "npu" / "rtlgen" / "out"
            vvp_path = root / "npu_top.vvp"
            cfg = json.loads(HARDSIGMOID_CFG.read_text(encoding="utf-8"))
            ...
            self.assertIn("VEC_OP_HARDSIGMOID = 4'hc", top_v)
            self.assertIn("vec_act_hardsigmoid_int8", top_v)
            self.assertIn("VEC_EN_HARDSIGMOID = 1", top_v)
