...
When generating a queue item from a disposable worktree, do not rely on an
editable-installed CLI script from `/workspaces/RTLGen` that points at a
different checkout.

Pin the developer-side Python import path to the source worktree when invoking
the CLI module:
