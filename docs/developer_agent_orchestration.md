...
When generating a queue item from a disposable worktree, import the control
plane package from that same worktree. Do not run
`PYTHONPATH=/workspaces/RTLGen/control_plane ... --repo-root /tmp/<worktree>`
unless `/workspaces/RTLGen` is the intended source checkout. Mixing a stale
`PYTHONPATH` with a fresh `--repo-root` can silently produce a work item whose
proposal metadata is current but whose command manifest was generated by old
code. Prefer:

```sh
PYTHONPATH=/tmp/<worktree>/control_plane \
  /workspaces/RTLGen/control_plane/.venv/bin/python \
  -m control_plane.cli.main generate-l2-campaign \
  --repo-root /tmp/<worktree> ...
```
