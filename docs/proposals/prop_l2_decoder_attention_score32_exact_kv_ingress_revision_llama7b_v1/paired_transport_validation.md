...
full p53 and p54 runs subsequently passed in 789.77 and 787.83 seconds,
respectively. Neither failed run supports a passing composed-path claim.

Each corrected pass covers one continuous path through real refill,
...
Both executed fixtures match `67a17cc9` after parameter substitution,
...
for head group zero of tiles 0, 1, and 2. It checks 69,632 refill writes,
394 completed descriptors, 12,288 ingress and transpose beats, and 24,576
producer-facing K/Q/last beats against the independent canonical oracle.
All correctness checks remain enabled under independent producer stalls.

The compiled testbench was compared byte-for-byte with the committed source
...
hardware latency. Earlier results above use the distinct noncanonical fixture.
