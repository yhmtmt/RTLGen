...
## Hypothesis
- a fresh proposal-backed hard-swish block under a unique canonical path is sufficient to prove that dashboard Resume can recover the submission without rerunning evaluation

## Evaluation Scope
- direct comparison set:
  - one standalone `int8` hard-swish `pwl` block only
- evaluation modes:
...
## Knowledge Inputs
- accepted hard-swish block proposal
- dashboard resume discussion
- prior retryproof submission recovery work

## Candidate Direction
- reuse the accepted hard-swish wrapper shape under a unique canonical wrapper path
