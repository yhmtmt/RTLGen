...
- produce an accepted reduced integrated physical source before any Layer 2
  LeakyReLU campaign is queued

## Constraints
- keep `nm1` fixed
- keep the first pass bounded to one integrated LeakyReLU-enabled path
- prefer hierarchy-preserving evaluation and avoid early full-top flattening
- use the accepted standalone `int8` LeakyReLU block as the lower-layer source

## Initial Plan
- add LeakyReLU vec op support to the integrated `nm1` generator/perf path
