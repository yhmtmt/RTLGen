...
// Exact K/Q stage with one 256-bit paired-dimension write per K bank cycle.
module attention_score32_exact_kv_key_stage_wide #(
  ...
  input wire [5:0] key_write_dimension_pair,
  ...
  reg [5:0] key_expected_pair_q [0:63];
  ...
  wire key_write_metadata_valid =
    ...
    key_write_dimension_pair == key_expected_pair_q[key_write_slot] &&
    key_write_last == (key_write_dimension_pair == 6'd63);
  ...
  wire [16383:0] key_bank_read_data;
  generate
    genvar bank;
    for (bank = 0; bank < 64; bank = bank + 1) begin : g_key_bank
      reg [255:0] bank_mem [0:63];
      always @(posedge clk) begin
        if (key_write_valid && key_write_ready && key_write_metadata_valid &&
            key_write_slot == bank)
          bank_mem[key_write_dimension_pair] <= key_write_data;
      end
      assign key_bank_read_data[(bank*256) +: 256] = bank_mem[command_beat_q[6:1]];
    end
  endgenerate
  ...
  generate
    ...
    for (producer = 0; producer < PRODUCERS; producer = producer + 1) begin : g_read
      ...
      wire [255:0] read_word_w = key_bank_read_data[(read_slot_w*256) +: 256];
      assign producer_query[(producer*128) +: 128] =
        {query_mem[command_beat_q[6:0]], query_mem[command_beat_q[6:0]]};
      assign producer_key[(producer*128) +: 128] = command_beat_q[0] ?
        read_word_w[255:128] : read_word_w[127:0];
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (clear_i = 0; clear_i < 64; clear_i = clear_i + 1)
        key_expected_pair_q[clear_i] <= 6'd0;
    end else begin
      command_done <= 1'b0;
      ...
      if (key_write_valid && key_write_ready) begin
        if (!key_write_metadata_valid) begin
          ...
          if (key_write_last)
            ...
            key_expected_pair_q[key_write_slot] <= key_expected_pair_q[key_write_slot] + 1'b1;
        end
      end
...
`ifndef SYNTHESIS
  initial begin
    if ((PRODUCERS != 53) && (PRODUCERS != 54)) begin
      $error("attention_score32_exact_kv_key_stage_wide PRODUCERS must be 53 or 54");
      $finish(1);
    end
  end
`endif
