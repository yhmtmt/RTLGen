...
def test_perf_vec_sigmoid():
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        bin_path = tmp / "vec_sigmoid.bin"
        out_path = tmp / "trace.json"
        cfg_path = tmp / "cfg.json"

        d0 = _pack_desc(0x11, flags=0x0A)
        payload = [0xC0, 0xE0, 0x00, 0x20, 0x40, 0x10, 0xF0, 0x7F]
    ...
    assert trace[0]["op"] == "sigmoid"
    assert trace[0]["expected_result_bytes"] == [0, 2, 8, 14, 16, 11, 5, 16]
    assert int(trace[0]["expected_result"], 0) >= 0


if __name__ == "__main__":
    test_perf_vec_sigmoid()
