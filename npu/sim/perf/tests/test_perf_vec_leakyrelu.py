...
def test_perf_vec_leakyrelu():
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        bin_path = tmp / "vec_leakyrelu.bin"
        out_path = tmp / "trace.json"
        cfg_path = tmp / "cfg.json"

        d0 = _pack_desc(0x11, flags=0x0F)
        payload = [0x80, 0xF0, 0xF8, 0xF9, 0xFF, 0x00, 0x01, 0x7F]
    ...
    assert trace[0]["op"] == "leakyrelu"
    assert trace[0]["expected_result_bytes"] == [240, 254, 255, 0, 0, 0, 1, 127]
    assert int(trace[0]["expected_result"], 0) >= 0


if __name__ == "__main__":
    test_perf_vec_leakyrelu()
