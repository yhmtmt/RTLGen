...
def test_perf_vec_hardtanh():
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        bin_path = tmp / "vec_hardtanh.bin"
        out_path = tmp / "trace.json"
        cfg_path = tmp / "cfg.json"

        d0 = _pack_desc(0x11, flags=0x0D)
    ...
    assert trace[0]["op"] == "hardtanh"
    assert trace[0]["expected_result_bytes"] == [240, 240, 240, 0, 16, 16, 16, 16]
    assert int(trace[0]["expected_result"], 0) >= 0


if __name__ == "__main__":
    test_perf_vec_hardtanh()
