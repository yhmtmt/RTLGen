...
def test_perf_vec_relu6():
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        bin_path = tmp / "vec_relu6.bin"
        out_path = tmp / "trace.json"
        cfg_path = tmp / "cfg.json"

        d0 = _pack_desc(0x11, flags=0x0E)
        payload = [0x80, 0xFF, 0x00, 0x01, 0x05, 0x06, 0x07, 0x7F]
    ...
    assert trace[0]["op"] == "relu6"
    assert trace[0]["expected_result_bytes"] == [0, 0, 0, 1, 5, 6, 6, 6]
