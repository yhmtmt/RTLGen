...
def test_perf_vec_tanh():
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        bin_path = tmp / "vec_tanh.bin"
        out_path = tmp / "trace.json"
        cfg_path = tmp / "cfg.json"

        d0 = _pack_desc(0x11, flags=0x0B)
        payload = [0xC0, 0xE0, 0xF0, 0x00, 0x10, 0x20, 0x40, 0x7F]
    ...
    assert trace[0]["op"] == "tanh"
    assert trace[0]["expected_result_bytes"] == [240, 240, 244, 0, 12, 16, 16, 16]
