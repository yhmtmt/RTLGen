...
def _pack_gemm_desc(path: Path):
    ...
    # A lanes (int16 little-endian): [1, -2, 3, -4]
    struct.pack_into("<hhhh", raw, 8, 1, -2, 3, -4)
    # B lanes (int16 little-endian): [5, 6, -7, -8]
    struct.pack_into("<hhhh", raw, 16, 5, 6, -7, -8)
    path.write_bytes(bytes(raw))


def test_perf_gemm_int16_expected_fields():
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        bin_path = tmp / "gemm_int16.bin"
...
if __name__ == "__main__":
    test_perf_gemm_int16_expected_fields()
