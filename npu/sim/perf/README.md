...
- `test_perf_vec_fp16.py`
  - Builds a synthetic fp16 `VEC_OP` stream (`add`, `mul`, `relu`) using dtype code `fp16`.
...
- Includes dedicated fp16 regressions for:
  ...
  - C++ RTLGen fp16 VEC path (`add/mul/relu`, dtype `fp16`), when FloPoCo is available
...
- fp16 coverage currently includes GEMM lane-1 IEEE-half + raw16 placeholder modes and perf-side fp16 VEC (`add/mul/relu`) expectation decode.
- bf16/fp8 and RTL fp16 VEC numeric equivalence are not covered yet.
