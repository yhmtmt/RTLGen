...
- Runs functional cross-check:
  ...
  - `compare_tensor_traces.py` compares RTL `TENSOR_TRACE` lines for GEMM accumulations and VEC results against perf-derived tensor summaries, then writes durable `*_tensor_trace_summary.json` artifacts.
