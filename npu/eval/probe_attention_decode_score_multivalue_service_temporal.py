#!/usr/bin/env python3
"""Probe the real c1 exact-partial service composed with temporal reduction."""
...
SEQUENCE_ID = 0x2233
LOGICAL_COMMAND_ID = 0x6A40
PHYSICAL_COMMAND_IDS = (0x5101, 0x5102)
HEAD_ID = 3
WINDOW_COUNT = 2
...
_SUMMARY_RE = re.compile(
    r"SUMMARY cycles=(\d+) commands=(\d+) second_refused=(\d+) first_terminal=(-?\d+) "
    r"second_accept=(-?\d+) outputs=(\d+) stable=(\d+) wrapper_error=(\d+) "
...
def _window_inputs(window_index: int) -> list[tuple[int, list[int]]]:
    return [
        beat
...
def _expected_rows() -> list[JsonDict]:
    ...
    for window_index, physical_command_id in enumerate(PHYSICAL_COMMAND_IDS):
        blocks = service_probe._cluster_beats(window_index)
        score_rows = [
            list(
        ...
        logical_beats = tuple(
            replace(beat, command_id=LOGICAL_COMMAND_ID) for beat in physical_beats
        )
        records.append(
            ExactPartialWindowRecord(
...
def _testbench(*, top_name: str, mismatch_metadata: bool) -> str:
    values = service_probe._shared_value_matrices()
    entries = service_probe._preload_entries(values)
    workload = service_probe._workload_contract()
    beats_per_window = int(service_probe._workload_expected_counts(workload)["input_beat_count"])
    all_beats = _window_inputs(0) + _window_inputs(1)
    preload_init = "\n".join(
...
module tb;
  ...
  localparam integer EXPECT_MISMATCH = {int(mismatch_metadata)};
  ...
  wire service_reassembler_protocol_error;
  ...
  reg [15:0] blocked_sequence;
  reg [4:0] blocked_head;
  reg [14:0] blocked_count;
  reg [15:0] blocked_command;
  reg signed [31:0] blocked_max;
  reg [32:0] blocked_sum;
  reg [3:0] blocked_slice;
  reg blocked_last;
  reg [327:0] blocked_value;

  assign out_ready = rst_n && (((cycle % 9) != 2) && ((cycle % 9) != 3) &&
                               ((cycle % 9) != 6));
  ...
  {top_name} dut (
      ...
      .service_reassembler_protocol_error(service_reassembler_protocol_error),
  ...
  always @* begin
    ...
    cluster_command_valid = rst_n && preload_done && (command_count < 2);
    cluster_command_id = (command_count == 0) ? 16'h{PHYSICAL_COMMAND_IDS[0]:04x}
                                              : 16'h{PHYSICAL_COMMAND_IDS[1]:04x};
    cluster_logical_sequence_id = 16'h{SEQUENCE_ID:04x};
    cluster_logical_command_id = 16'h{LOGICAL_COMMAND_ID:04x};
    ...
    cluster_command_head_id = 5'd{HEAD_ID};
    cluster_command_score_multiplier = 32'd1;
    cluster_command_score_shift = 6'd0;

    cluster_input_valid = rst_n && input_active;
    ...
    cluster_input_last = input_active &&
        (((input_index + 1) % 128) == 0);
  end
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (cluster_command_valid && cluster_command_ready) begin
        if (command_count == 1) second_accept_cycle <= cycle;
      ...
      if ((first_terminal_cycle < 0) && service_shared_result_valid &&
          service_shared_result_ready && service_shared_result_last) begin
        first_terminal_cycle <= cycle;
      end
{mismatch_block}
      if (out_valid && !out_ready) begin
        if (!blocked_output_valid) begin
          blocked_output_valid <= 1'b1;
          blocked_sequence <= out_sequence_id;
          blocked_head <= out_head_id;
          blocked_count <= out_window_count;
          blocked_command <= out_command_id;
          blocked_max <= out_global_max;
          blocked_sum <= out_exp_sum;
          blocked_slice <= out_slice;
          blocked_last <= out_last;
          blocked_value <= out_value;
        end else begin
          if ((out_sequence_id !== blocked_sequence) ||
              (out_head_id !== blocked_head) ||
              (out_window_count !== blocked_count) ||
              (out_command_id !== blocked_command) ||
              (out_global_max !== blocked_max) ||
              (out_exp_sum !== blocked_sum) ||
              (out_slice !== blocked_slice) ||
              (out_last !== blocked_last) ||
              (out_value !== blocked_value)) begin
            $fatal(1, "full temporal output changed under backpressure");
          end
      ...
      if (out_valid && out_ready) begin
        $display("OUT sequence=%0d head=%0d count=%0d command=%0d max=%0d sum=%0d slice=%0d last=%0d value=%082x",
                 ...
                 $signed(out_global_max), out_exp_sum, out_slice, out_last, out_value);
        output_count <= output_count + 1;
      end

      if ({completion_condition}) begin
        $display("SUMMARY cycles=%0d commands=%0d second_refused=%0d first_terminal=%0d second_accept=%0d outputs=%0d stable=%0d wrapper_error=%0d service_error=%0d temporal_error=%0d protocol_error=%0d service_accepted=%0d service_completed=%0d service_req=%0d service_resp=%0d temporal_inputs=%0d temporal_merges=%0d temporal_emitted=%0d temporal_heads=%0d output_stalls=%0d",
                 cycle, command_count, second_refused_cycles, first_terminal_cycle,
                 ...
                 service_cluster_accepted_count, service_cluster_completed_count,
                 service_accepted_req_count, service_emitted_resp_count,
                 temporal_input_accepted_count, temporal_merge_completed_count,
                 temporal_emitted_beat_count, temporal_completed_head_count,
                 temporal_output_stall_cycles);
        $finish;
      end
      if (cycle > 150000) $fatal(1, "service temporal probe timeout");
    end
  end
...
def build_report(*, mismatch_metadata: bool = False) -> JsonDict:
    top_name = "attention_decode_score_multivalue_service_temporal_probe"
    config = _config(top_name)
    expected = [] if mismatch_metadata else _expected_rows()
    with tempfile.TemporaryDirectory(prefix="decode-service-temporal-probe-") as temp_name:
        temp = Path(temp_name)
        rtl_dir = temp / "rtl"
        generate(config, rtl_dir)
        tb_path = temp / "tb.sv"
        tb_path.write_text(
            _testbench(top_name=top_name, mismatch_metadata=mismatch_metadata),
            encoding="utf-8",
        )
        simv = temp / "simv"
        ...
        manifest = json.loads(
            (rtl_dir / _MANIFEST_NAME).read_text(encoding="utf-8")
        )

    if mismatch_metadata:
        passed = (
            summary["wrapper_error"] == 1
            and summary["protocol_error"] == 1
            and summary["service_error"] == 0
            and summary["temporal_error"] == 0
    ...
    return {
        "model": "attention_decode_score_multivalue_service_temporal_probe_v1",
        "passed": passed,
        "mismatch_metadata": mismatch_metadata,
        "physical_command_ids": list(PHYSICAL_COMMAND_IDS),
        "logical_sequence_id": SEQUENCE_ID,
...
def main() -> int:
    ...
    report = build_report(mismatch_metadata=args.mismatch_metadata)
