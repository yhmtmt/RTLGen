...
from llm_reference import build_reference_fixture
...
def main() -> int:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else manifest_path.parent / 'reference'
    out_dir.mkdir(parents=True, exist_ok=True)
    out_manifest = Path(args.out_manifest) if args.out_manifest else manifest_path.parent / 'reference_manifest.json'
    ...
    for model in model_manifest.get('models', []):
        ...
        fixture = build_reference_fixture(onnx_path, model_id=model_id)
        ...
        entries.append(
            {
                ...
                'fixture_json': _repo_friendly(fixture_path),
    ...
    print(f'gen_llm_reference_suite: wrote {len(entries)} fixtures to {out_dir}')
    print(f'gen_llm_reference_suite: wrote manifest to {out_manifest}')
    return 0
