#!/usr/bin/env python3
"""Probe the exact-partial reduction tree with an embodied root finalizer."""
...
_FINALIZER_RE = re.compile(
    r"FINALIZER_SUMMARY accepted=(\d+) completed=(\d+) protocol_error=(\d+) tree_root_completed=(\d+) tree_protocol_error=(\d+)"
)
...
def _expected(clusters: int, *, heads: int, divider_lanes: int) -> dict[str, object]:
    ...
    finalizer_reference = simulate_exact_finalizer(
        merged,
        divider_lanes=divider_lanes,
        output_ready_pattern=tuple(((cycle % 5) != 2) and (((cycle + 3) % 11) != 7) for cycle in range(64)),
    )
    return {
        "heads": heads,
        "leaves": leaf_rows,
        ...
        "standalone_finalizer_reference": finalizer_reference,
    }


def _config(clusters: int, divider_lanes: int) -> dict[str, object]:
    return {
...
def _testbench(*, top_name: str, clusters: int, heads: int, divider_lanes: int, expected: dict[str, object]) -> str:
    ...
    timeout_cycles = max(
        4000,
        (clusters * total_results * 3) + (total_results * (((8 // divider_lanes) * 70) + 20)),
    )
...
module tb;
  ...
  always @* begin
    ...
    root_ready = ((cycle % 5) != 2) && (((cycle + 3) % 11) != 7);
...
{chr(10).join(stage_displays)}
        $display("FINALIZER_SUMMARY accepted=%0d completed=%0d protocol_error=%0d tree_root_completed=%0d tree_protocol_error=%0d",
                 finalizer_accepted_count, root_completed_count, finalizer_protocol_error, tree_root_completed_count, tree_protocol_error);
...
def build_report(clusters: int = 16, heads: int = 32, divider_lanes: int = 8) -> JsonDict:
    expected = _expected(clusters, heads=heads, divider_lanes=divider_lanes)
    with tempfile.TemporaryDirectory(prefix=f"score32_exact_finalized_tree_c{clusters}_l{divider_lanes}_") as temp_dir_name:
        temp_dir = Path(temp_dir_name)
        generate_tree(_config(clusters, divider_lanes), temp_dir / "rtl")
        tb_path = temp_dir / "tb.sv"
        top_name = str(_config(clusters, divider_lanes)["top_name"])
        tb_path.write_text(
            _testbench(
                top_name=top_name,
                clusters=clusters,
        ...
        run = subprocess.run([_tool("vvp"), str(simv)], capture_output=True, text=True, timeout=600)
        if run.returncode:
    ...
    if summary is None or finalizer_summary is None:
        raise RuntimeError("summary missing from finalized tree probe")
    ...
    measured_workload_manifest = exact_finalized_tree_service_manifest(
        clusters=clusters,
        heads=heads,
        divider_lanes=divider_lanes,
    )
    ...
    return {
        ...
        "standalone_finalizer_reference": expected["standalone_finalizer_reference"],
...
def main() -> int:
    ...
    report = build_report(clusters=args.clusters, heads=args.heads, divider_lanes=args.divider_lanes)
    if args.json:
