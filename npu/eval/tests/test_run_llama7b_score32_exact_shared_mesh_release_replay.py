...
PASS_LINE = (
    ...
    "release_vc1_done_cycle=17000 release_overlap_valid=0 release_overlap_arb=0 "
    "release_arb_decisions=70948 "
...
def test_result_retains_completion_split_and_exact_contract(tmp_path: Path) -> None:
    ...
    assert result["rtl_observation"]["inter_vc_overlap_valid_cycles"] == 0
    assert result["rtl_observation"]["inter_vc_overlap_arbitrated_cycles"] == 0
    ...
    assert "canonical addressed K/V ingress" in result["next_gate"]
    assert "backpressure through full representative K and V heads" in result["next_gate"]
...
def test_result_preserves_nonzero_overlap_for_other_cadences(tmp_path: Path) -> None:
    cadence_path = tmp_path / "cadence.json"
    cadence_path.write_text(json.dumps(_cadence()), encoding="utf-8")
    replay = build_replay(_cadence(), cadence_path=cadence_path)
    stdout = PASS_LINE.replace("release_overlap_valid=0", "release_overlap_valid=12").replace(
        "release_overlap_arb=0", "release_overlap_arb=9"
    )
    result = build_result(replay=replay, cadence_path=cadence_path, pytest_stdout=stdout)
    assert result["rtl_observation"]["inter_vc_overlap_valid_cycles"] == 12
    assert result["rtl_observation"]["inter_vc_overlap_arbitrated_cycles"] == 9
    assert "valid cycles: `12`" in render_markdown(result)
