...
POST_FLOORPLAN_TIE_ROOT_FIX_TCL_NAME = "post_floorplan_tie_root_fix.tcl"
...
def build_post_floorplan_tie_root_fix_tcl() -> str:
    return """# Auto-generated by npu/synth/run_block_sweep.py.
...
proc rtlgen_parse_tie_cell_and_port {env_var} {
  ...
  set spec [string trim $::env($env_var)]
  if {$spec eq ""} {
    return {}
  }
  set parts $spec
  if {[llength $parts] < 2} {
    error "Expected $env_var to contain '<cell> <port>', got '$spec'"
  }
  set cell_name [lindex $parts 0]
  set port_name [lindex $parts 1]
  set lib_cells [get_lib_cell $cell_name]
  if {[llength $lib_cells] == 0} {
    error "Unable to resolve tie cell '$cell_name' from $env_var"
  }
  set lib_cell [lindex $lib_cells 0]
  set lib_name [get_name [get_property $lib_cell library]]
  return [list $lib_name $cell_name $port_name]
}

proc rtlgen_sanitize_name {text} {
  set sanitized [regsub -all {[^A-Za-z0-9_]} $text _]
  if {$sanitized eq ""} {
    set sanitized "net"
  }
  if {![regexp {^[A-Za-z_]} $sanitized]} {
    set sanitized "n_$sanitized"
  }
  return $sanitized
}

proc rtlgen_unique_tie_instance_name {block prefix net_name} {
  set base "${prefix}_[rtlgen_sanitize_name $net_name]"
  set candidate $base
  set suffix 0
  while {[$block findInst $candidate] != "NULL"} {
    incr suffix
...
proc rtlgen_count_net_sinks_and_drivers {net} {
  set sink_count 0
  set output_driver_count 0

  foreach iterm [$net getITerms] {
    set io_type [string toupper [[$iterm getMTerm] getIoType]]
    if {$io_type eq "INPUT"} {
      incr sink_count
    } elseif {$io_type eq "OUTPUT" || $io_type eq "INOUT" || $io_type eq "FEEDTHRU"} {
      incr output_driver_count
    }
  }

  foreach bterm [$net getBTerms] {
    set io_type [string toupper [$bterm getIoType]]
    if {$io_type eq "INPUT"} {
      incr output_driver_count
    } elseif {$io_type eq "OUTPUT"} {
      incr sink_count
    } elseif {$io_type eq "INOUT" || $io_type eq "FEEDTHRU"} {
      incr sink_count
      incr output_driver_count
    }
  }

  return [list $sink_count $output_driver_count]
}

proc rtlgen_insert_residual_tie_roots_for_sigtype {block sig_type env_var prefix} {
  set tie_spec [rtlgen_parse_tie_cell_and_port $env_var]
  if {[llength $tie_spec] != 3} {
    return 0
  }
  lassign $tie_spec lib_name cell_name port_name
  set candidates {}
  ...
  foreach net [$block getNets] {
    ...
    if {[string equal -nocase $net_name "VDD"] || [string equal -nocase $net_name "VSS"]} {
      continue
    }
    if {[string toupper [$net getSigType]] ne $sig_type} {
      continue
    }
    lappend candidates [list $net_name $net]
  }

  set inserted 0
  foreach candidate [lsort -index 0 $candidates] {
    lassign $candidate net_name net
    lassign [rtlgen_count_net_sinks_and_drivers $net] sink_count output_driver_count
    if {$sink_count <= 0 || $output_driver_count > 0} {
      continue
    }
    set inst_name [rtlgen_unique_tie_instance_name $block $prefix $net_name]
...
proc rtlgen_insert_residual_tie_roots {} {
  set block [ord::get_db_block]
  if {$block == "NULL"} {
    return
  }
  set inserted_ground [rtlgen_insert_residual_tie_roots_for_sigtype $block GROUND TIELO_CELL_AND_PORT rtlgen_auto_tielo]
  set inserted_power [rtlgen_insert_residual_tie_roots_for_sigtype $block POWER TIEHI_CELL_AND_PORT rtlgen_auto_tiehi]
  puts "Residual tie-root fix complete: inserted [expr {$inserted_ground + $inserted_power}] tie roots"
}

rtlgen_insert_residual_tie_roots
"""


def write_post_floorplan_tie_root_fix_tcl(path: Path) -> None:
    path.write_text(build_post_floorplan_tie_root_fix_tcl(), encoding="utf-8")
...
def ensure_design_assets(
    ...
    if force or not dest_platform_dir.is_dir():
        ...
        post_floorplan_tcl_path = dest_platform_dir / POST_FLOORPLAN_TIE_ROOT_FIX_TCL_NAME
        write_post_floorplan_tie_root_fix_tcl(post_floorplan_tcl_path)
        config_lines.append(
            "export POST_FLOORPLAN_TCL = "
            f"$(DESIGN_HOME)/{platform}/{design_name}/{POST_FLOORPLAN_TIE_ROOT_FIX_TCL_NAME}"
        )
