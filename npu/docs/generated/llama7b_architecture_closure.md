...
Evidence:
- `npu/docs/generated/llama7b_physically_credible_pareto.json` (generated_audit; `routed_ppa`, `composition`, `scale_validation`): Computes three-objective dominance only among quality-backed promotable rows, identifies separate score32 latency/area and exact-FP16 energy anchors, excludes abstract/quality-invalid dominators, proves the score32 latency lead survives the full exact serialized RMSNorm sensitivity envelope, and fail-closes full-model and activity-backed promotion.
