...
- provisional because: The recorded 12.814 ms score32 point consumes a superseded 986-cycle tile-service assumption. With measured local-reducer service, single-clock bounds are 146.083--172.388 ms, while the proposed dual-clock bounds are 29.361--50.588 ms before RMSNorm and require unmeasured CDC/scheduler composition; energy is not activity-closed, so score32 Pareto membership is currently unproven.
...
- `docs/proposals/prop_l1_decoder_llama7b_rmsnorm_phase3_macro_banked_physical_v1/evaluation_requests.json` (proposal_gate; `routed_ppa`, `activity`): Defines a matched guarded routed-PPA comparison of the conservative and three-credit exact 64-macro hierarchies; implementation is merged and dispatch remains pending human approval.
...
| `composition` | `open` | The final topology/scheduler composition remains open. Ascending K-plane delivery conflicts with the paired-stream transpose contract. An opt-in paired scheduler matches all 2,113,984 full-model descriptors and propagates widened counters through the mesh wrapper. Bounded nonuniform-payload real-refill-to-mesh-to-transpose-to-K/Q tests pass for p53 and p54 over three heads including a resident/HBM split, checking 24,576 producer-facing beats per family. The same three-tile path now also passes with canonical tensor inputs and an independent producer oracle for both families. A separate bounded canonical composition now connects paired K scheduling, transpose, K/Q staging, V ingress, and generated banked SRAM service to one selected numerical producer for each family across four groups; 512 exact rows per family pass. Separate concurrent p54 endpoint-0 and p53 endpoint-8 replays now connect real paired K scheduling, transpose, K/Q staging and V ingress to all producers and generated SRAM across four groups and eight waves. All 512 canonical rows per endpoint match the reference and direct-sidecar baseline, with 44 source and 19 generated-input hashes verified. Serialized testbench memory delivery gives last-output cycles 201903 and 201908; these are diagnostic cycles, not workload latency. This diagnostic evaluation does not connect the mesh to that numerical composition, establish full-model mapped attention equivalence, or close workload latency, technology SRAM mapping, or physical PPA. |
...
- status: `open`
- confidence: `low`
- summary: The previously reported two-point component-composed Pareto set is retracted as a physically credible set because its 12.814 ms score32 point consumes the superseded 986-cycle tile-service assumption. The latest accepted recost includes measured p53 eight-wave local-reducer service: its inherited single-clock interval is 146.083--172.388 ms and loses to the 72.544 ms exact-FP16 reference, while its proposed dual-clock interval is 29.361--50.588 ms before RMSNorm and 30.034--52.694 ms across the serialized RMSNorm envelope. The dual-clock latency lead is conditional on unmeasured CDC/scheduler composition. Routed local-reducer submacro evidence adds 52.754--92.347 mm2 across 16 clusters, leaving 90.429--130.022 mm2 of the old area headroom before other missing score32-only area, but energy remains unrecost and not activity-backed. Exact FP16 is therefore retained as the current measured reference while score32 membership is unproven.
...
| `scale_validation` | `open` | Measured local-reducer service produces distinct single- and dual-clock Llama7B intervals, but no integrated activity-backed rerank consumes that evidence and the dual-clock result still lacks CDC/scheduler closure. |
...
- The dual-clock latency interval must not be promoted until its CDC and safe-overlap scheduler are implemented and measured; the single-clock interval is slower than exact FP16.
- Measured local-reducer area is a submacro replication/lower-bound interval, not routed composed-top area, and must not be paired with the old score32 energy as if they were one evaluated design point.
...
Evidence:
- `npu/docs/generated/llama7b_score32_cadence_frontier_consistency.json` (generated_audit; `composition`, `scale_validation`): Proves that the recorded score32 point consumes the superseded 986-cycle term; consumes the later measured local-reducer recost; distinguishes single-clock 146.083--172.388 ms from CDC-dependent dual-clock 29.361--50.588 ms; carries the serialized RMSNorm and local-reducer area intervals; and retracts score32 Pareto membership pending composition and activity closure.
