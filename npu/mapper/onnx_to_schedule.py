...
def _infer_terminal_vecop_graph(
    ...
    *,
    ...
    tail_consumers = consumers.get(current_name, [])
    terminal_vec_nodes = [n for n in tail_consumers if n.op_type in {"Relu", "Relu6", "LeakyRelu", "Sigmoid", "Tanh", "HardSigmoid", "HardTanh"}]
    if len(terminal_vec_nodes) != 1 or len(tail_consumers) != 1:
        raise ValueError(
            f"expected a single terminal Relu, Relu6, LeakyRelu, Sigmoid, Tanh, HardSigmoid, or HardTanh consumer on {current_name!r}, "
