...
PAIR_CAPTURE_TO_OUTPUT_LATENCY_CYCLES = 20
PAIR_COMPUTE_LAUNCH_TO_OUTPUT_LATENCY_CYCLES = 19
...
def _validate(config: dict[str, Any]) -> dict[str, int | str | bool]:
    ...
    if not top_name or not isinstance(body, dict):
        raise SystemExit("config requires top_name and attention_score32_exact_partial_pair_merge_folded")
    ...
    keep_hierarchy_explicit = "keep_hierarchy" in body
    ...
    return {
        ...
        "scale_divider_impl_explicit": scale_divider_impl_explicit,
        "keep_hierarchy": keep_hierarchy,
        "keep_hierarchy_explicit": keep_hierarchy_explicit,
    }
...
{hierarchy_attr}module {top_name} (
  ...
  reg signed [40:0] shared_signed_value_r;
  reg [23:0] shared_signed_scale_r;
  reg [32:0] shared_unsigned_value_r;
  reg [23:0] shared_unsigned_scale_r;
...
{_lane_write_cases()}
  ...
  wire signed [40:0] shared_signed_scaled_w =
      scale_signed41(shared_signed_value_r, shared_signed_scale_r);
  wire [32:0] shared_unsigned_scaled_w =
  ...
  always @(*) begin
    ...
    shared_signed_value_r = 41'sd0;
    shared_signed_scale_r = 24'd0;
    shared_unsigned_value_r = 33'd0;
    shared_unsigned_scale_r = 24'd0;
    case (phase_q)
      PHASE_EXP_SUM_LEFT: begin
        shared_unsigned_value_r = active_left_exp_sum_q;
        shared_unsigned_scale_r = active_left_scale_q;
      end
      PHASE_EXP_SUM_RIGHT: begin
        shared_unsigned_value_r = active_right_exp_sum_q;
        shared_unsigned_scale_r = active_right_scale_q;
      end
      PHASE_LANE_LEFT: begin
        shared_signed_value_r = lane_read41(active_left_value_q, active_lane_index_q);
        shared_signed_scale_r = active_left_scale_q;
      end
      PHASE_LANE_RIGHT: begin
        shared_signed_value_r = lane_read41(active_right_value_q, active_lane_index_q);
        shared_signed_scale_r = active_right_scale_q;
      end
      default: begin
      end
    endcase
...
def generate(config: dict[str, Any], out_dir: Path) -> None:
    ...
    if bool(params["keep_hierarchy_explicit"]):
        manifest["keep_hierarchy"] = bool(params["keep_hierarchy"])
