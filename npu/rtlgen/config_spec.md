...
- `compute.vec.activation_source` (string): activation backend selector for Phase 2.
  ...
  - `rtlgen_cpp`: import activation modules generated by C++ RTLGen (`relu`/`gelu` now).
...
- `activation_source=rtlgen_cpp` currently emits only `relu` and `gelu` modules.
  Planned Phase-2/Phase-3 follow-up is to add `softmax`, `layernorm`, and
  derivative kernels in C++ RTLGen and then consume them from NPU RTLGen.
