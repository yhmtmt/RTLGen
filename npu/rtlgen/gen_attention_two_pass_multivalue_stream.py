...
def _partial_result_cases(value_slices: int) -> str:
    cases = []
    for value_slice in range(value_slices):
        assignments = "\n".join(
            f"          result_value[{lane * 41} +: 41] <= numerator_accum[{value_slice * 8 + lane}];"
            for lane in range(8)
        )
        cases.append(f"        {value_slice}: begin\n{assignments}\n        end")
    return "\n".join(cases)
...
def _top(*, top_name: str, max_blocks: int, value_slices: int, fsm_encoding: str) -> str:
    ...
    if fsm_encoding == "binary":
        state_decl = '  (* fsm_encoding = "binary" *) reg [3:0] state;'
    elif fsm_encoding == "explicit_onehot":
        state_params = _onehot_state_declarations(state_names=_STATE_NAMES, state_name="state")
        state_decl = ""
...
def _top_exact_partial(
    *,
    ...
    if fsm_encoding == "binary":
        state_decl = '  (* fsm_encoding = "binary" *) reg [2:0] state;'
    elif fsm_encoding == "explicit_onehot":
        state_params = _onehot_state_declarations(state_names=_PARTIAL_STATE_NAMES, state_name="state")
        state_decl = ""
    return f"""// Auto-generated by npu/rtlgen/gen_attention_two_pass_multivalue_stream.py
module {top_name} (
    input  wire         clk,
    ...
    input  wire         result_ready,
    output reg  [15:0]  result_command_id,
    output reg  [{head_id_bits - 1}:0] result_head_id,
    output reg  signed [31:0] result_global_max,
    output reg  [32:0]  result_exp_sum,
    output reg  [{slice_bits - 1}:0] result_slice,
    output wire         result_last,
...
{state_decl}
  reg [15:0] active_command_id;
  reg [{head_id_bits - 1}:0] active_head_id;
  reg [{count_bits - 1}:0] active_block_count;
  reg [{count_bits - 1}:0] fill_count;
  reg [{addr_bits - 1}:0] read_index;
  reg [{slice_bits - 1}:0] value_slice_index;
...
{_exp_cases()}
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      state <= IDLE;
      active_command_id <= 16'd0;
      active_head_id <= {head_id_bits}'d0;
      active_block_count <= 0;
      fill_count <= 0;
      read_index <= 0;
      value_slice_index <= 0;
      ...
      result_valid <= 1'b0;
      result_command_id <= 16'd0;
      result_head_id <= {head_id_bits}'d0;
      result_global_max <= 32'sd0;
      result_exp_sum <= 33'd0;
      result_slice <= 0;
      result_value <= 328'd0;
...
{_accumulate_cases(value_slices)}
          ...
          if (value_slice_index == VALUE_SLICES - 1) begin
            if (read_index + 1'b1 == active_block_count) begin
              ...
              result_slice <= 0;
              case (0)
{_partial_result_cases(value_slices)}
              endcase
              state <= RESULT;
            end else begin
              read_index <= read_index + 1'b1;
...
def generate(config: dict[str, Any], out_dir: Path) -> None:
    ...
    manifest = {
        ...
        "semantic_profile": (
            "q8_k8_v8_a32_s32_exp_lut_b20_shared_score_multivalue_exact_partial_v1"
            if result_mode == "exact_partial"
            else "q8_k8_v8_a32_s32_exp_lut_b20_shared_score_multivalue_iterdiv_v1"
        ),
        ...
        "state_width": (
            len(_PARTIAL_STATE_NAMES)
            if result_mode == "exact_partial" and params["fsm_encoding"] == "explicit_onehot"
            else len(_STATE_NAMES)
            if params["fsm_encoding"] == "explicit_onehot"
            else 3
            if result_mode == "exact_partial"
            else 4
        ),
        "state_names": list(_PARTIAL_STATE_NAMES if result_mode == "exact_partial" else _STATE_NAMES),
        "divider_units": 0 if result_mode == "exact_partial" else 1,
        "divider_cycles_per_command": 0 if result_mode == "exact_partial" else value_slices * 8 * 60,
        "result_interface": (
            "ready_valid_exact_partial_slice_stream"
            if result_mode == "exact_partial"
            else "ready_valid_8d_slice_stream"
        ),
        "result_beats_per_command": value_slices,
        "accumulator_clear_cycles": value_slices,
    }
    if result_mode == "exact_partial":
        manifest["result_value_bits_per_beat"] = 328
