...
Layout
------
- `runs/designs/<circuit_type>/<design>/config.json` — the config that produced the metrics (e.g., `runs/designs/prefix_adders/adder_ripple_4u_wrapper/config.json`).
- `runs/designs/<circuit_type>/<design>/verilog/*.v` — generated RTL for that config (no DEF/GDS/logs).
- `runs/designs/<circuit_type>/<design>/metrics.csv` — aggregated execution parameters and parsed metrics per run; new sweeps append rows.
- `runs/designs/<circuit_type>/<design>/work/` — transient per-run scratch (may be cleared without losing the published artifacts).
...
- Summaries: `runs/campaigns/prefix_adders/nangate45_unsigned_summary.csv`, `runs/campaigns/prefix_adders/sky130hd_unsigned_summary.csv`, `runs/campaigns/prefix_adders/asap7_unsigned_summary.csv`.
- Sweep settings: small blocks use `CORE_UTILIZATION=10` (and relaxed variants for stubborn cases) to avoid PDN/route failures.

Multipliers:
- Width sweep (Nangate45/Sky130HD/ASAP7): 4/8/16/32-bit, signed+unsigned, PPG {Normal, Booth4}, CPA {Ripple, BrentKung, KoggeStone, Sklansky}.
  - Summaries: `runs/campaigns/multipliers/ppg_cpa_widths_4_32/nangate45_summary.csv`, `runs/campaigns/multipliers/ppg_cpa_widths_4_32/sky130hd_summary.csv`, `runs/campaigns/multipliers/ppg_cpa_widths_4_32/asap7_summary.csv` (64 rows each); replaces earlier 16-bit-only pilot sweeps.
...
- Yosys Booth baseline comparison: `runs/campaigns/multipliers/yosys_booth_widths_4_32` holds Booth/LowpowerBooth (signed-only for lowpower) sweeps. Plots `analysis/ilp_over_yosys_*` normalize ILP to Yosys (so <1.0 favors ILP). Yosys “Booth” behaves CLA-like (compare against ILP prefix CPAs), while “LowpowerBooth” looks RCA-like (compare against ILP Ripple/BrentKung). ILP prefix CPAs generally beat Yosys Booth in delay (area close on some wide signed Nangate45), and ILP ripple-style generally beats Yosys LowpowerBooth in delay with similar or smaller area; ASAP7 shows the largest ILP advantage.

Contribute
----------
- Add a new config under `runs/campaigns/<circuit_type>/` (or a variant name) and open a PR; keep configs minimal and documented.
- Run `scripts/run_sweep.py ... --out_root runs/designs/<circuit_type>` locally; it will append rows to `metrics.csv` and refresh `verilog/`.
