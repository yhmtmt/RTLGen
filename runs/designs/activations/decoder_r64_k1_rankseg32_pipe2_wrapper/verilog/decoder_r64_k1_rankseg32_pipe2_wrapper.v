`timescale 1ns/1ps
module decoder_r64_k1_rankseg32_pipe2_wrapper(
  input clk,
  input rst_n,
  input in_valid,
  output in_ready,
  ...
  wire [9:0] local_indices;
  wire signed [31:0] local_logits;
  logit_rank_r32_l16_k1 local_rank_0 (
    .logits(masked_logits[0 +: 512]),
    .top_indices(local_indices[0 +: 5]),
    .top_logits(local_logits[0 +: 16])
  );
  logit_rank_r32_l16_k1 local_rank_1 (
    .logits(masked_logits[512 +: 512]),
    .top_indices(local_indices[5 +: 5]),
    .top_logits(local_logits[16 +: 16])
  );
  wire merge_in_ready;
  reg stage1_valid;
  reg stage1_last;
  reg signed [31:0] stage1_logits;
  reg [31:0] stage1_token_ids;
  reg stage2_valid;
  reg stage2_last;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (token_i = 0; token_i < 2; token_i = token_i + 1) begin
        stage1_token_ids[token_i*16 +: 16] <= in_base_token_id + token_i*32 + { 11'd0, local_indices[token_i*5 +: 5] };
      end
    end
  end
  wire [0:0] global_index;
  wire signed [15:0] global_logit;
  logit_rank_r2_l16_k1 global_rank (
  ...
  always @* begin
    global_token_id = '0;
    case (global_index)
      1'd0: global_token_id = stage1_token_ids[0 +: 16];
      1'd1: global_token_id = stage1_token_ids[16 +: 16];
