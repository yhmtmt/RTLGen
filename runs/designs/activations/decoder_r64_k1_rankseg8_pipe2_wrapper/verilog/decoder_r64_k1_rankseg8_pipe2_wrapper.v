`timescale 1ns/1ps
module decoder_r64_k1_rankseg8_pipe2_wrapper(
  input clk,
  input rst_n,
  input in_valid,
  output in_ready,
  ...
  wire [23:0] local_indices;
  wire signed [127:0] local_logits;
  logit_rank_r8_l16_k1 local_rank_0 (
    .logits(masked_logits[0 +: 128]),
    .top_indices(local_indices[0 +: 3]),
    .top_logits(local_logits[0 +: 16])
  );
  logit_rank_r8_l16_k1 local_rank_1 (
    .logits(masked_logits[128 +: 128]),
    .top_indices(local_indices[3 +: 3]),
    .top_logits(local_logits[16 +: 16])
  );
  logit_rank_r8_l16_k1 local_rank_2 (
    .logits(masked_logits[256 +: 128]),
    .top_indices(local_indices[6 +: 3]),
    .top_logits(local_logits[32 +: 16])
  );
  logit_rank_r8_l16_k1 local_rank_3 (
    .logits(masked_logits[384 +: 128]),
    .top_indices(local_indices[9 +: 3]),
    .top_logits(local_logits[48 +: 16])
  );
  logit_rank_r8_l16_k1 local_rank_4 (
    .logits(masked_logits[512 +: 128]),
    .top_indices(local_indices[12 +: 3]),
    .top_logits(local_logits[64 +: 16])
  );
  logit_rank_r8_l16_k1 local_rank_5 (
    .logits(masked_logits[640 +: 128]),
    .top_indices(local_indices[15 +: 3]),
    .top_logits(local_logits[80 +: 16])
  );
  logit_rank_r8_l16_k1 local_rank_6 (
    .logits(masked_logits[768 +: 128]),
    .top_indices(local_indices[18 +: 3]),
    .top_logits(local_logits[96 +: 16])
  );
  logit_rank_r8_l16_k1 local_rank_7 (
    .logits(masked_logits[896 +: 128]),
    .top_indices(local_indices[21 +: 3]),
    .top_logits(local_logits[112 +: 16])
  );
  wire merge_in_ready;
  reg stage1_valid;
  reg stage1_last;
  ...
  reg stage2_valid;
  reg stage2_last;
  reg signed [15:0] stage2_logit;
  reg [15:0] stage2_token_id;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (token_i = 0; token_i < 8; token_i = token_i + 1) begin
        stage1_token_ids[token_i*16 +: 16] <= in_base_token_id + token_i*8 + { 13'd0, local_indices[token_i*3 +: 3] };
      end
    end
  end
  wire [2:0] global_index;
  wire signed [15:0] global_logit;
  logit_rank_r8_l16_k1 global_rank (
    .logits(stage1_logits),
    .top_indices(global_index),
    .top_logits(global_logit)
  );
  reg [15:0] global_token_id;
  always @* begin
    global_token_id = '0;
    case (global_index)
      3'd0: global_token_id = stage1_token_ids[0 +: 16];
      3'd1: global_token_id = stage1_token_ids[16 +: 16];
      3'd2: global_token_id = stage1_token_ids[32 +: 16];
      3'd3: global_token_id = stage1_token_ids[48 +: 16];
      3'd4: global_token_id = stage1_token_ids[64 +: 16];
      3'd5: global_token_id = stage1_token_ids[80 +: 16];
      3'd6: global_token_id = stage1_token_ids[96 +: 16];
      3'd7: global_token_id = stage1_token_ids[112 +: 16];
      default: global_token_id = stage1_token_ids[0 +: 16];
    endcase
  end
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
