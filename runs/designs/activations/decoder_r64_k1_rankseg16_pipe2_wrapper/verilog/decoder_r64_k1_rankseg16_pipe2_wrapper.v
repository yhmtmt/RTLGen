`timescale 1ns/1ps
module decoder_r64_k1_rankseg16_pipe2_wrapper(
  input clk,
  input rst_n,
  input in_valid,
  output in_ready,
  ...
  wire [15:0] local_indices;
  wire signed [63:0] local_logits;
  logit_rank_r16_l16_k1 local_rank_0 (
    .logits(masked_logits[0 +: 256]),
    .top_indices(local_indices[0 +: 4]),
    .top_logits(local_logits[0 +: 16])
  );
  logit_rank_r16_l16_k1 local_rank_1 (
    .logits(masked_logits[256 +: 256]),
    .top_indices(local_indices[4 +: 4]),
    .top_logits(local_logits[16 +: 16])
  );
  logit_rank_r16_l16_k1 local_rank_2 (
    .logits(masked_logits[512 +: 256]),
    .top_indices(local_indices[8 +: 4]),
    .top_logits(local_logits[32 +: 16])
  );
  logit_rank_r16_l16_k1 local_rank_3 (
    .logits(masked_logits[768 +: 256]),
    .top_indices(local_indices[12 +: 4]),
    .top_logits(local_logits[48 +: 16])
  );
  wire merge_in_ready;
  reg stage1_valid;
  reg stage1_last;
  reg signed [63:0] stage1_logits;
  reg [63:0] stage1_token_ids;
  reg stage2_valid;
  reg stage2_last;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (token_i = 0; token_i < 4; token_i = token_i + 1) begin
        stage1_token_ids[token_i*16 +: 16] <= in_base_token_id + token_i*16 + { 12'd0, local_indices[token_i*4 +: 4] };
      end
    end
  end
  wire [1:0] global_index;
  wire signed [15:0] global_logit;
  logit_rank_r4_l16_k1 global_rank (
  ...
  always @* begin
    global_token_id = '0;
    case (global_index)
      2'd0: global_token_id = stage1_token_ids[0 +: 16];
      2'd1: global_token_id = stage1_token_ids[16 +: 16];
      2'd2: global_token_id = stage1_token_ids[32 +: 16];
      2'd3: global_token_id = stage1_token_ids[48 +: 16];
