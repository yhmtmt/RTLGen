...
module softmax_rowwise_int8_r4_wrapper(
  ...
  softmax_rowwise_int8_r4 u_core (
    .X(x_reg),
    .Y(y_wire)
  );
...
module gemm_compute_array (
  ...
  localparam integer NUM_MODULES = 2;
  localparam integer LANES = 1;
  localparam integer ELEM_BITS = 16;
  ...
  generate
    for (mi = 0; mi < NUM_MODULES; mi = mi + 1) begin : g_slot
      ...
      wire [15:0] slot_mac_next16;
      wire signed [ACC_WIDTH-1:0] slot_next;

      gemm_mac_fp16_ieee u_slot_mac (
        .A(slot_a[15:0]),
        .B(slot_b[15:0]),
...
,
  ...
  reg vec_pending;
  reg vec_done_pulse;
  reg [31:0] softmax_in_row;
  reg [31:0] softmax_last_result;
  ...
  localparam       SOFTMAX_DESC_ENABLED = 1;
  localparam integer SOFTMAX_ROW_BYTES = 4;
  localparam integer SOFTMAX_DATA_WIDTH = 32;
  localparam integer GEMM_NUM_MODULES = 2;
  ...
  wire signed [31:0] gemm_slot_next_flat;
  (* keep_hierarchy = 1 *) gemm_compute_array u_gemm_compute_array (
    .slot_a_flat({gemm_mac_a_vec1, gemm_mac_a_vec0}),
    .slot_b_flat({gemm_mac_b_vec1, gemm_mac_b_vec0}),
    .slot_accum_in_flat({gemm_slot_accum1, gemm_slot_accum0}),
  ...
  wire [31:0] softmax_result_next;
  (* keep_hierarchy = 1 *) softmax_rowwise_int8_r4_wrapper u_softmax_engine (
    .clk(clk),
    .X(softmax_in_row),
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // 2-slot GEMM stub with OOO-capable completion scheduling.
      ...
      if (gemm_slot_valid[0] && !gemm_slot_done[0]) begin
        gemm_slot_accum0 <= $signed(gemm_slot_next_flat[(0*16) +: 16]);
      end
      if (gemm_slot_valid[1] && !gemm_slot_done[1]) begin
        gemm_slot_accum1 <= $signed(gemm_slot_next_flat[(1*16) +: 16]);
      end
      if (gemm_slot_valid[0] && !gemm_slot_done[0]) begin
      ...
      if (!dma_pending) begin
        ...
        else if (gemm_slot_done[0]) begin
      ...
      gemm_pending <= (gemm_slot_valid != 2'b0);
      m_axi_awvalid <= 1'b0;
      m_axi_awaddr <= 0;
      m_axi_awlen <= 0;
      m_axi_awsize <= 0;
          ...
          if (cq_mem_rdata[23:16] >= 8'h02) begin
            ...
            if (cq_mem_rdata[7:0] == 8'h01) begin
              ...
              if (!gemm_slot_valid[0]) begin
                ...
                gemm_slot_size0 <= ((cq_mem_rdata[11:8] == 4'h1 || cq_mem_rdata[11:8] == 4'h2) ? (cq_mem_rdata[63:52] * cq_mem_rdata[51:42] * 2) : (cq_mem_rdata[63:52] * cq_mem_rdata[51:42]));
                gemm_slot_beats0 <= (((((cq_mem_rdata[11:8] == 4'h1 || cq_mem_rdata[11:8] == 4'h2) ? (cq_mem_rdata[63:52] * cq_mem_rdata[51:42] * 2) : (cq_mem_rdata[63:52] * cq_mem_rdata[51:42]))) + AXI_BEAT_BYTES - 1) >> 5);
                gemm_slot_arlen0 <= ((((((cq_mem_rdata[11:8] == 4'h1 || cq_mem_rdata[11:8] == 4'h2) ? (cq_mem_rdata[63:52] * cq_mem_rdata[51:42] * 2) : (cq_mem_rdata[63:52] * cq_mem_rdata[51:42]))) + AXI_BEAT_BYTES - 1) >> 5) - 1);
              end
              else if (!gemm_slot_valid[1]) begin
                ...
                gemm_slot_size1 <= ((cq_mem_rdata[11:8] == 4'h1 || cq_mem_rdata[11:8] == 4'h2) ? (cq_mem_rdata[63:52] * cq_mem_rdata[51:42] * 2) : (cq_mem_rdata[63:52] * cq_mem_rdata[51:42]));
                gemm_slot_beats1 <= (((((cq_mem_rdata[11:8] == 4'h1 || cq_mem_rdata[11:8] == 4'h2) ? (cq_mem_rdata[63:52] * cq_mem_rdata[51:42] * 2) : (cq_mem_rdata[63:52] * cq_mem_rdata[51:42]))) + AXI_BEAT_BYTES - 1) >> 5);
                gemm_slot_arlen1 <= ((((((cq_mem_rdata[11:8] == 4'h1 || cq_mem_rdata[11:8] == 4'h2) ? (cq_mem_rdata[63:52] * cq_mem_rdata[51:42] * 2) : (cq_mem_rdata[63:52] * cq_mem_rdata[51:42]))) + AXI_BEAT_BYTES - 1) >> 5) - 1);
              end
              else begin
              ...
              if (!SOFTMAX_DESC_ENABLED) begin
                ...
                softmax_in_row <= cq_mem_rdata[95:64];
                softmax_last_result <= 0;
            ...
            if (cq_word0[7:0] == 8'h10) begin
              ...
              if (!gemm_slot_valid[0]) begin
                ...
                if ((cq_mem_rdata[31:0] == 0) || (cq_mem_rdata[63:32] == 0) || (cq_mem_rdata[95:64] == 0)) begin
                  ...
                  gemm_slot_cycles0 <= (((cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * cq_mem_rdata[95:64]) >> 10) + 1 + (cq_mem_rdata[255] ? 8 : 0));
                end
                gemm_slot_src0 <= cq_word0[127:64];
                gemm_slot_dst0 <= cq_word0[255:192];
                gemm_slot_size0 <= ((cq_word0[11:8] == 4'h1 || cq_word0[11:8] == 4'h2) ? (cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * 2) : (cq_mem_rdata[31:0] * cq_mem_rdata[63:32]));
                gemm_slot_beats0 <= (((((cq_word0[11:8] == 4'h1 || cq_word0[11:8] == 4'h2) ? (cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * 2) : (cq_mem_rdata[31:0] * cq_mem_rdata[63:32]))) + AXI_BEAT_BYTES - 1) >> 5);
                gemm_slot_arlen0 <= ((((((cq_word0[11:8] == 4'h1 || cq_word0[11:8] == 4'h2) ? (cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * 2) : (cq_mem_rdata[31:0] * cq_mem_rdata[63:32]))) + AXI_BEAT_BYTES - 1) >> 5) - 1);
              end
              else if (!gemm_slot_valid[1]) begin
                gemm_slot_valid[1] <= 1'b1;
                gemm_slot_done[1] <= 1'b0;
                ...
                if ((cq_mem_rdata[31:0] == 0) || (cq_mem_rdata[63:32] == 0) || (cq_mem_rdata[95:64] == 0)) begin
                  ...
                  gemm_slot_cycles1 <= (((cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * cq_mem_rdata[95:64]) >> 10) + 1 + (cq_mem_rdata[255] ? 8 : 0));
                end
                gemm_slot_src1 <= cq_word0[127:64];
                gemm_slot_dst1 <= cq_word0[255:192];
                gemm_slot_size1 <= ((cq_word0[11:8] == 4'h1 || cq_word0[11:8] == 4'h2) ? (cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * 2) : (cq_mem_rdata[31:0] * cq_mem_rdata[63:32]));
                gemm_slot_beats1 <= (((((cq_word0[11:8] == 4'h1 || cq_word0[11:8] == 4'h2) ? (cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * 2) : (cq_mem_rdata[31:0] * cq_mem_rdata[63:32]))) + AXI_BEAT_BYTES - 1) >> 5);
                gemm_slot_arlen1 <= ((((((cq_word0[11:8] == 4'h1 || cq_word0[11:8] == 4'h2) ? (cq_mem_rdata[31:0] * cq_mem_rdata[63:32] * 2) : (cq_mem_rdata[31:0] * cq_mem_rdata[63:32]))) + AXI_BEAT_BYTES - 1) >> 5) - 1);
              end
              else begin
                error_code <= 32'h2; // GEMM in-flight queue full
              end
      ...
      if (error_code != 0) begin
        ...
        if ((cq_count != 0) || cq_stage_valid || dma_pending || vec_pending || softmax_pending || (gemm_slot_valid != 2'b0)) begin
          status <= STATUS_BUSY;
        end else begin
          status <= STATUS_IDLE;
        end
