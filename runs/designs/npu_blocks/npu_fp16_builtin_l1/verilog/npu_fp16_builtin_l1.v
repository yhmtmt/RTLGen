...
module gemm_compute_array (
    ...
    input  wire signed [63:0] slot_accum_in_flat,
    output wire signed [63:0] slot_accum_next_flat
);
  ...
  generate
    for (mi = 0; mi < NUM_MODULES; mi = mi + 1) begin : g_slot
      ...
      wire signed [ACC_WIDTH-1:0] slot_mac_dot;
...
,
  ...
  wire signed [63:0] gemm_slot_next_flat;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (gemm_slot_valid[0] && !gemm_slot_done[0]) begin
        gemm_slot_accum0 <= $signed(gemm_slot_next_flat[(0*32) +: 32]);
      end
      if (gemm_slot_valid[1] && !gemm_slot_done[1]) begin
        gemm_slot_accum1 <= $signed(gemm_slot_next_flat[(1*32) +: 32]);
      end
      if (gemm_slot_valid[0] && !gemm_slot_done[0]) begin
      ...
      if (error_code != 0) begin
        ...
        if ((cq_count != 0) || cq_stage_valid || dma_pending || vec_pending || (gemm_slot_valid != 2'b0)) begin
          status <= STATUS_BUSY;
        end else begin
          status <= STATUS_IDLE;
        end
