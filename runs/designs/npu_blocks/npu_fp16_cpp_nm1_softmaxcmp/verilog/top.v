...
module gemm_compute_array (
    input  wire [15:0] slot_a_flat,
    input  wire [15:0] slot_b_flat,
    input  wire signed [15:0] slot_accum_in_flat,
    output wire signed [15:0] slot_accum_next_flat
);
  localparam integer NUM_MODULES = 1;
...
,
  ...
  reg [2:0]  dma_state;
  reg        gemm_pending;
  reg [0:0]  gemm_slot_valid;
  reg [0:0]  gemm_slot_done;
  ...
  localparam integer GEMM_NUM_MODULES = 1;
  ...
  wire signed [15:0] gemm_slot_next_flat;
  (* keep_hierarchy = 1 *) gemm_compute_array u_gemm_compute_array (
    .slot_a_flat(gemm_mac_a_vec0),
    .slot_b_flat(gemm_mac_b_vec0),
    .slot_accum_in_flat(gemm_slot_accum0),
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // 1-slot GEMM stub with OOO-capable completion scheduling.
      ...
      gemm_pending <= (gemm_slot_valid != 1'b0);
      m_axi_awvalid <= 1'b0;
      m_axi_awaddr <= 0;
      m_axi_awlen <= 0;
      m_axi_awsize <= 0;
      ...
      if (error_code != 0) begin
        ...
        if ((cq_count != 0) || cq_stage_valid || dma_pending || vec_pending || softmax_pending || (gemm_slot_valid != 1'b0)) begin
          status <= STATUS_BUSY;
        end else begin
          status <= STATUS_IDLE;
        end
