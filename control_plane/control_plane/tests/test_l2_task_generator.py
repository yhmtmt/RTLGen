...
def test_generate_l2_campaign_task_adds_schedule_wrapper_postroute_activity_power_input() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        with Session(engine) as session:
            ...
            assert work_item.acceptance_rules == [
...
def test_generate_l2_campaign_task_adds_operational_component_frontier() -> None:
    with tempfile.TemporaryDirectory() as td:
        repo_root = Path(td) / "repo"
        repo_root.mkdir()
        campaign_path = _write_campaign(repo_root)
        ...
        with Session(engine) as session:
            result = generate_l2_campaign_task(
                ...
                _make_l2_request(
                    ...
                    evaluation_mode="frontier_recost",
                    depends_on_item_ids=[
                        "l1_decoder_attention_operational_dense_tile_ppa_v1",
            ...
            assert command["name"] == "audit_decoder_attention_operational_component_frontier"
            assert "--operational-tile-metrics-csv" in command["run"]
            assert "--score-bank-metrics-csv" in command["run"]
            assert decoder_inputs["operational_dense_tile_metrics"].endswith(
                "dense_gemm_tile_stream_int8_16x8/metrics.csv"
            )
            assert decoder_inputs["score_bank_proxy_metrics"].endswith(
                "attention_score_bank_proxy_16kx256/metrics.csv"
            )
...
def test_generate_l2_campaign_task_adds_decode_score_tile_equivalence() -> None:
    with tempfile.TemporaryDirectory() as td:
        repo_root = Path(td) / "repo"
        repo_root.mkdir()
        campaign_path = _write_campaign(repo_root)
        ...
        with Session(engine) as session:
            ...
            assert command["name"] == "audit_decode_score_tile_equivalence"
            assert "attention_decode_score_tile_int8_1x8" in command["run"]
            assert "test_packed_decode_score_tile_matches_reference_and_protocol" in command["run"]
            assert decoder_inputs["decode_score_tile_equivalence_out"].endswith(
                "l2_decoder_attention_decode_score_tile_m1x8_equivalence_llama7b_v1.json"
            )


def test_generate_l2_campaign_task_adds_decode_score_local_cluster_equivalence() -> None:
    with tempfile.TemporaryDirectory() as td:
        repo_root = Path(td) / "repo"
        repo_root.mkdir()
        campaign_path = _write_campaign(repo_root)
        ...
        with Session(engine) as session:
            ...
            assert command["name"] == "probe_decode_score_local_cluster_equivalence"
            assert "-m npu.eval.probe_attention_decode_score_local_cluster_equivalence" in command["run"]
            assert "attention_decode_score_local_cluster_int8_m1x8_iterdiv/config.json" in command["run"]
            assert decoder_inputs["decode_score_local_cluster_equivalence_out"].endswith(
...
def test_generate_l2_campaign_task_adds_decode_score_multivalue_gqa_group_equivalence() -> None:
    with tempfile.TemporaryDirectory() as td:
        repo_root = Path(td) / "repo"
        repo_root.mkdir()
        campaign_path = _write_campaign(repo_root)
        ...
        with Session(engine) as session:
            ...
            work_item = session.query(WorkItem).filter_by(item_id=result.item_id).one()
            payload = work_item.task_request.request_payload
            command = payload["task"]["commands"][0]
            decoder_inputs = work_item.input_manifest["decoder_contract"]

            assert command["name"] == "audit_decode_score_multivalue_gqa_group_equivalence"
            ...
            assert work_item.source_commit == source_commit
            assert decoder_inputs["decode_score_multivalue_gqa_group_equivalence_out"].endswith(
...
def test_generate_l2_campaign_task_adds_decode_score_local_cluster_frontier() -> None:
    with tempfile.TemporaryDirectory() as td:
        repo_root = Path(td) / "repo"
        repo_root.mkdir()
        campaign_path = _write_campaign(repo_root)
        ...
        with Session(engine) as session:
            ...
            assert command["name"] == "audit_decode_score_local_cluster_frontier"
            assert "audit_llm_decoder_attention_decode_score_local_cluster_frontier" in command["run"]
            assert decoder_inputs["decode_score_local_cluster_frontier_out"].endswith(
                "l2_decoder_attention_decode_score_local_cluster_frontier_llama7b_v1.json"
            )


def test_generate_l2_campaign_task_adds_decode_score_multivalue_cluster_frontier() -> None:
    with tempfile.TemporaryDirectory() as td:
        repo_root = Path(td) / "repo"
        repo_root.mkdir()
        campaign_path = _write_campaign(repo_root)
        ...
        with Session(engine) as session:
            ...
            assert [command["name"] for command in commands[:2]] == [
                "audit_decode_score_multivalue_cluster_frontier",
                "validate_runs",
            ]
            assert (
                "-m npu.eval.audit_attention_decode_score_multivalue_cluster_frontier" in run
            )
            assert (
                "--prior-frontier-json "
                "runs/datasets/llm_decoder_eval_gpt2_prompt_stress_v1/"
                "decoder_attention_decode_score_local_cluster_frontier__"
...
def test_generate_l2_campaign_task_adds_decode_score_multivalue_gqa_group_frontier() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        with Session(engine) as session:
            ...
            assert [command["name"] for command in commands[:2]] == [
                "audit_decode_score_multivalue_gqa_group_frontier",
                "validate_runs",
            ]
            assert "-m npu.eval.audit_attention_decode_score_multivalue_gqa_group_frontier" in run
            assert (
                "--prior-frontier-json runs/datasets/llm_decoder_eval_gpt2_prompt_stress_v1/"
                "decoder_attention_decode_score_multivalue_cluster_frontier__"
                "l2_decoder_attention_decode_score_multivalue_cluster_frontier_llama7b_v1.json"
            ) in run
            assert (
                "--group-activity-power-json runs/datasets/llm_decoder_eval_gpt2_prompt_stress_v1/"
                "decoder_attention_decode_score_multivalue_gqa_group_activity_power__"
                "l2_decoder_attention_decode_score_multivalue_gqa8_group_activity_power_llama7b_v1.json"
            ) in run
            assert "--group-counts 1,2,4" in run
            assert decoder_inputs["decode_score_multivalue_gqa_group_frontier_group_counts"] == "1,2,4"
            assert "linearly composed, not array-PNR measured" in decoder_inputs[
                "decode_score_multivalue_gqa_group_frontier_scope"
            ]
            assert decoder_inputs["decode_score_multivalue_gqa_group_frontier_out"].endswith(
                "decoder_attention_decode_score_multivalue_gqa_group_frontier__"
...
def test_generate_l2_campaign_task_adds_score32_noc_measured_router_closure() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        with Session(engine) as session:
            ...
            assert "l1_segmented_xy_mesh_noc_phase1_v1_r7.json" in run
