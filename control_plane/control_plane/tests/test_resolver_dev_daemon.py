...
def test_dev_resolver_keeps_source_mismatch_when_exact_run_predates_case() -> None:
    engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
    create_all(engine)
    _seed_assigned_ready_source_mismatch(engine)
    ...
    with Session(engine) as session:
        case = session.query(ResolverCase).filter(ResolverCase.id == "case-newer-source-mismatch").one()

    assert result.poll_count == 1
    assert fetch_issue_mock.call_count == 1
    assert close_issue_mock.call_count == 0
    assert case.status == "awaiting_remote"
    assert case.resolution_json == {}
