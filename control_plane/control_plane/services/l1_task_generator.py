    ...
    *,
    ...
    if "multiplier" in cfg:
        ...
        if top_name != "noc_segmented_mesh_router_node5":
            raise Layer1TaskGenerationError(
                f"bare router top must be noc_segmented_mesh_router_node5 in {config_path}"
            )
        try:
            design_dir = str(config_path.parent.resolve().relative_to(repo_root.resolve()))
        except ValueError as exc:
            raise Layer1TaskGenerationError(
                f"bare router physical config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        verilog_dir = f"{design_dir}/verilog"
        return Layer1ConfigTarget(
            ...
            commands=[
                {
                    "name": "stage_noc_segmented_mesh_router_bare_rtl",
                    "run": _with_oss_cad_path(
                        "python3 npu/rtlgen/stage_noc_segmented_mesh_router_bare.py "
                        f"--config {config_rel} --out {verilog_dir}"
                    ),
                },
                {
                    "name": "check_noc_segmented_mesh_router_bare_guard",
                    "run": _with_oss_cad_path(
                        "python3 npu/eval/check_noc_segmented_mesh_router_bare_guard.py "
                        f"--config {config_rel} --verilog-dir {verilog_dir}"
                    ),
                },
                {
                ...
                {
                    "name": "extract_noc_segmented_mesh_router_bare_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} --out {design_dir}/timing_debug_report.md --max-paths 8"
                    ),
    ...
    elif "top_name" in cfg and "attention_score32_exact_finalizer_bank_control" in cfg:
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                f"score32 exact finalizer bank control config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        return Layer1ConfigTarget(
            design_kind="block",
            ...
            commands=[
                {
                    "name": "generate_attention_score32_exact_finalizer_bank_control_rtl",
                    "run": _with_oss_cad_path(
                ...
                {
                    "name": "check_attention_score32_exact_finalizer_bank_control_guard",
                    "run": (
                ...
                {
                    "name": "extract_attention_score32_exact_finalizer_bank_control_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} --out {design_dir}/timing_debug_report.md --max-paths 8"
                    ),
    ...
    elif "top_name" in cfg and "attention_score32_exact_partial_pair_merge_folded" in cfg:
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                f"folded score32 exact partial pair-merge config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        return Layer1ConfigTarget(
            design_kind="block",
            ...
            commands=[
                {
                    "name": "generate_attention_score32_exact_partial_pair_merge_folded_rtl",
                    "run": _with_oss_cad_path(
                ...
                {
                    "name": "check_attention_score32_exact_partial_pair_merge_folded_guard",
                    "run": (
                        "python3 npu/eval/check_attention_score32_exact_partial_pair_merge_guard.py "
                        f"--design-dir {design_dir} --config {config_rel} --sweep {{sweep_path}}"
                    ),
                },
                {
                ...
                {
                    "name": "extract_attention_score32_exact_partial_pair_merge_folded_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} --out {design_dir}/timing_debug_report.md --max-paths 8"
                    ),
        ...
        try:
            ...
            raise Layer1TaskGenerationError(
                f"decode-score multivalue GQA-group config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        macro_manifest = f"{design_dir}/macro_manifest.json"
        return Layer1ConfigTarget(
            ...
            commands=[
                {
                    "name": "generate_attention_decode_score_multivalue_gqa_group_rtl",
                    "run": _with_oss_cad_path(
                        "python3 npu/rtlgen/gen_attention_decode_score_multivalue_gqa_group.py "
                        f"--config {config_rel} --out {design_dir}/verilog"
                    ),
                },
                {
                    "name": "check_attention_decode_score_multivalue_gqa_group_guard",
                    "run": (
                        "python3 npu/eval/check_attention_decode_score_multivalue_gqa_group_guard.py "
                        f"--design-dir {design_dir}"
                    ),
                },
                {
                ...
                {
                    "name": "extract_attention_decode_score_multivalue_gqa_group_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} --out {design_dir}/timing_debug_report.md --max-paths 8"
                    ),
    ...
    elif "top_name" in cfg and "attention_decode_score_multivalue_cluster" in cfg:
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                f"decode-score multivalue-cluster config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        macro_manifest = f"{design_dir}/macro_manifest.json"
        return Layer1ConfigTarget(
            ...
            commands=[
                {
                    "name": "generate_attention_decode_score_multivalue_cluster_rtl",
                    "run": _with_oss_cad_path(
                        "python3 npu/rtlgen/gen_attention_decode_score_multivalue_cluster.py "
                        f"--config {config_rel} --out {design_dir}/verilog"
                    ),
                },
                {
                ...
                {
                    "name": "extract_attention_decode_score_multivalue_cluster_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} --out {design_dir}/timing_debug_report.md --max-paths 8"
                    ),
    ...
    elif "top_name" in cfg and "attention_decode_score_local_cluster" in cfg:
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                f"decode-score local-cluster config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        macro_manifest = f"{design_dir}/macro_manifest.json"
        return Layer1ConfigTarget(
            ...
            commands=[
                {
                    "name": "generate_attention_decode_score_local_cluster_rtl",
                    "run": _with_oss_cad_path(
                        "python3 npu/rtlgen/gen_attention_decode_score_local_cluster.py "
                        f"--config {config_rel} --out {design_dir}/verilog"
                    ),
                },
                {
                    "name": "check_attention_decode_score_local_cluster_guard",
                    "run": (
                        "python3 npu/eval/check_attention_decode_score_local_cluster_guard.py "
                        f"--design-dir {design_dir}"
                    ),
                },
                {
                ...
                {
                    "name": "extract_attention_decode_score_local_cluster_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} --out {design_dir}/timing_debug_report.md --max-paths 8"
                    ),
    ...
    elif "top_name" in cfg and "attention_dual_stream_schedule_wrapper" in cfg:
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                f"attention dual-stream schedule wrapper config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        return Layer1ConfigTarget(
            design_kind="block",
            ...
            commands=[
                {
                    "name": "generate_attention_dual_stream_schedule_wrapper_rtl",
                    "run": _with_oss_cad_path(
                        (
                ...
                {
                    "name": "check_attention_dual_stream_schedule_wrapper_guard",
                    "run": (
                ...
                {
                    "name": "extract_attention_dual_stream_schedule_wrapper_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} "
                        f"--out {design_dir}/timing_debug_report.md "
    ...
    elif "top_name" in cfg and "attention_command_dispatch" in cfg:
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                f"attention command dispatch config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        return Layer1ConfigTarget(
            design_kind="block",
            ...
            commands=[
                {
                    "name": "generate_attention_command_dispatch_rtl",
                    "run": _with_oss_cad_path(
                        (
                ...
                {
                    "name": "check_attention_command_dispatch_guard",
                    "run": (
                ...
                {
                    "name": "extract_attention_command_dispatch_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} "
                        f"--out {design_dir}/timing_debug_report.md "
        ...
        "top_name" in cfg
        and "attention_score32_exact_kv_key_ingress_ppa_harness" in cfg
    ):
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                "exact K ingress physical config must live under "
                f"repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        command_slug = "attention_score32_exact_kv_key_ingress_ppa_harness"
        return Layer1ConfigTarget(
            design_kind="block",
            design_name=design_name,
            expected_metrics_path=block_metrics_path(design_name),
            ...
            commands=[
                {
                    ...
                    "run": _with_oss_cad_path(
                        (
                            "python3 npu/rtlgen/"
                            "gen_attention_score32_exact_kv_key_ingress_ppa_harness.py "
                            f"--config {config_rel} "
                            f"--out {design_dir}/verilog"
                        )
                    ),
                ...
                {
                    "name": "check_attention_score32_exact_kv_key_ingress_ppa_guard",
                    "run": (
                        "python3 npu/eval/"
                        "check_attention_score32_exact_kv_key_ingress_ppa_guard.py "
                        f"--design-dir {design_dir}"
                    ),
                },
                {
        ...
        "top_name" in cfg
        and "attention_shared_stream_context_service_ppa_activity_harness" in cfg
    ):
        top_name = str(cfg["top_name"]).strip()
        if not top_name:
            raise Layer1TaskGenerationError(f"top_name must not be empty in {config_path}")
        try:
            ...
            raise Layer1TaskGenerationError(
                "shared-stream context-service physical config must live under "
                f"repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        command_slug = "attention_shared_stream_context_service_ppa_activity_harness"
        return Layer1ConfigTarget(
            design_kind="block",
            design_name=design_name,
            expected_metrics_path=block_metrics_path(design_name),
            ...
            commands=[
                {
                    ...
                    "run": _with_oss_cad_path(
                        "python3 npu/rtlgen/"
                        "gen_attention_shared_stream_context_service_ppa_activity_harness.py "
                        f"--config {config_rel} --out {design_dir}/verilog"
                    ),
                },
                {
                    ...
                    "run": (
                        "python3 npu/eval/"
                        "check_attention_shared_stream_context_service_ppa_guard.py "
                        f"--design-dir {design_dir}"
                    ),
                },
                {
        ...
        try:
            ...
            raise Layer1TaskGenerationError(
                "exact shared-root transport activity physical config must live under "
                f"repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        command_slug = "attention_score32_exact_shared_root_transport_ppa_activity_harness"
        return Layer1ConfigTarget(
            design_kind="block",
            design_name=design_name,
            expected_metrics_path=block_metrics_path(design_name),
            ...
            commands=[
                {
                    ...
                    "run": _with_oss_cad_path(
                        "python3 npu/rtlgen/"
                        "gen_attention_score32_exact_shared_root_transport_ppa_activity_harness.py "
                        f"--config {config_rel} --out {design_dir}/verilog"
                    ),
                },
                {
                    ...
                    "run": (
                        "python3 npu/eval/"
                        "check_attention_score32_exact_shared_root_transport_ppa_activity_guard.py "
                        f"--design-dir {design_dir}"
                    ),
                },
                {
        ...
        try:
            ...
            raise Layer1TaskGenerationError(
                f"attention HBM replay controller config must live under repo_root/runs/designs/...: {config_path}"
            ) from exc
        design_name = config_path.parent.name
        return Layer1ConfigTarget(
            design_kind="block",
            ...
            commands=[
                {
                    "name": "generate_attention_hbm_replay_controller_rtl",
                    "run": _with_oss_cad_path(
                        (
                ...
                {
                    "name": "check_attention_hbm_replay_controller_guard",
                    "run": (
                ...
                {
                    "name": "extract_attention_hbm_replay_controller_timing_paths",
                    "run": (
                        "python3 npu/eval/extract_openroad_timing_summary.py "
                        f"--design-dir {design_dir} "
                        f"--out {design_dir}/timing_debug_report.md "
