...
def _decoder_attention_decode_score_tile_equivalence_evidence(*, item_id: str) -> dict[str, Any]:
    base = "runs/datasets/llm_decoder_eval_gpt2_prompt_stress_v1"
    out = f"{base}/decoder_attention_decode_score_tile_equivalence__{item_id}.json"
    report = f"{base}/decoder_attention_decode_score_tile_equivalence__{item_id}.md"
    return {
        "inputs": {
            "decode_score_tile_equivalence_out": out,
            "decode_score_tile_equivalence_report": report,
            "decode_score_tile_equivalence_scope": (
                "Prove exact signed-int8 M=1,N=8 decoder QK accumulation, final-beat inclusion, packed score-row "
                "lane order, clear/reuse, and ready/valid backpressure behavior against the dot-product reference."
            ),
        },
        "commands": [
            {
                ...
                "run": (
                    ...
                    "--test-target tests/test_dense_gemm_tile_stream.py::"
                    "test_packed_decode_score_tile_matches_reference_and_protocol "
...
def _decoder_attention_decode_score_multivalue_cluster_equivalence_evidence(*, item_id: str) -> dict[str, Any]:
    base = "runs/datasets/llm_decoder_eval_gpt2_prompt_stress_v1"
    out = f"{base}/decoder_attention_decode_score_multivalue_cluster_equivalence__{item_id}.json"
    report = f"{base}/decoder_attention_decode_score_multivalue_cluster_equivalence__{item_id}.md"
    config = (
        "runs/designs/npu_blocks/attention_decode_score_multivalue_cluster_int8_m1x8_iterdiv/"
        "config.json"
    )
    return {
        "inputs": {
            "decode_score_multivalue_cluster_equivalence_out": out,
            "decode_score_multivalue_cluster_equivalence_report": report,
            "decode_score_multivalue_cluster_equivalence_scope": (
                "Prove one M1x8 score-row fill per 8-token block, shared score storage, 16 eight-dimensional "
                "value-slice replays, streamed eight-dimension result beats, and one iterative divider preserve "
                "the signed-int8 Q/K/V, s32 score, LUT-softmax integer contract."
            ),
        },
        "commands": [
            {
                "name": "probe_decode_score_multivalue_cluster_equivalence",
                "run": (
                    "python3 -m npu.eval.probe_attention_decode_score_multivalue_cluster_equivalence "
                    f"--config {config} --out {out} --out-md {report}"
                ),
            }
        ],
    ...
    return {
        "inputs": {
            ...
            "attention_score32_exact_local16_global_tree_cluster_sram_gqa8_equivalence_scope": (
                "Run the fine-grained concrete compositional GQA8 cluster-SRAM RTL equivalence probe "
                "under bounded remote resources. Require the strict generated-top guard, serial exact "
